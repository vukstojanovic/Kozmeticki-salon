...
export default function EditDeleteService({
    ...
    return (
        <Flex justifyContent="space-between" alignItems='center' m='auto' w='2xl' _hover={{ bg: 'gray.50' }}>
            <Text paddingLeft='15px'>{name}</Text>
            <Box bg='white' paddingLeft='20px'>
                <Button
                    bg='white' color="white" onClick={onEditServiceOpen} borderRadius='unset' _hover={{ transform: 'scale(1.13)', filter: "brightness(140%)", }}>
                    <FaRegEdit color="green" size={20} />
                </Button>
                <Button onClick={onDeleteServiceOpen}
                    bg='white'
                    color="white"
                    marginLeft="10px" borderRadius='unset' _hover={{ transform: 'scale(1.13)', filter: "brightness(140%)", }}>

                    <IoTrash color="red" size={20} />
                </Button>
            </Box>
            <Modal isOpen={isEditServiceOpen} blockScrollOnMount={false} onClose={onEditServiceClose} >
                <ModalOverlay />
                <ModalContent>
                    ...
                    <ModalBody>
                        <form onSubmit={handleSubmit((data) => updateService({ data, id }))}>
                            <Box>
                                ...
                                <Text pl='20px' fontSize='12' opacity='0.8'>Category</Text>
                                <SelectChakra {...register("category_id", { valueAsNumber: true })} mb='20px' defaultValue={category_id}>
                                ...
                                <Input
                                    ...
                                    {...register("time_in_minutes", { valueAsNumber: true, required: 'Minutes are required' })} defaultValue={time_in_minutes}
                                />
                                {errors.time_in_minutes && <Text color='red.500' mt={'-17px'} ml='20px' mb='15px'>{errors.time_in_minutes.message}</Text>}

                                <Text pl='20px' fontSize='12' opacity='0.8'>Workers</Text>
                                <Controller
                                    control={control}
                                    ...
                                    //@ts-ignore
                                    defaultValue={servicesData && servicesData.data ?
                                        servicesData.data
                                            .find((service) => service.id === id)?.workers_id
                                            ?.map((workerId) => ({
                                                value: workerId,
                                                label: workersData?.data?.find((worker) => worker.id === workerId)?.name || `Worker ${workerId}`,
                                            })) || []
                                        : []}
                                />
                                <FormLabel htmlFor="price" pl='20px' fontSize='12' marginTop='20px' marginBottom='2px' opacity='0.8'>Price</FormLabel>
                                <Input
                                    id="price"
                                    fontSize={16}
                                    border="none"
                                    ...
                                    {...register("price", { valueAsNumber: true, required: 'Minutes are required' })}
                                    defaultValue={price}
                                />
            ...
            >
                ...
                <ModalContent>
                    ...
                    <ModalBody>
                        <Box>
                            <p>Are you sure you want to  <span style={{ fontWeight: 'bold' }}>delete "{name}"</span>. This action cannot be undone.</p>
                            <Flex justifyContent='flex-end' marginTop='20px'>
                                <Button onClick={onDeleteServiceClose} fontWeight='normal'>Cancel</Button>
                                <Button
