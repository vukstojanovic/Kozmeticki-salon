...
const Home = () => {
    ...
    return (
        <Grid maxW={{ base: "100%", md: "720px", lg: "1200px" }} h={'auto'} bgColor='white' paddingTop={{ base: '0', md: '100px' }}  >
            <Img src={beauty} alt='logo' h={'full'} opacity='80%' px={{ base: "0", md: '0' }} display={{ base: 'block', md: 'none' }} />
            {
                categoriesData?.data?.map((category) => {
                    return (
                        <>
                            <Box key={category.id} paddingBottom={{ base: "10px", md: '50px' }} px={{ base: "10px", md: '0' }}>
                                ...
                                <Grid templateColumns={{ base: "repeat(1, 1fr)", md: "repeat(3, 1fr)" }} gap='15px' >
                                    {filteredServices
                                        ...
                                        .map(filteredService => (
                                            <Flex key={filteredService.id} fontSize='xl' paddingBottom='20px' justifyContent={{ base: "space-between", md: "normal" }} gap='10px' flexDirection={{ base: 'row', md: 'column' }}>
                                                <Text paddingBottom='5px' fontFamily='monospace' fontWeight={"semibold"} color='blackAlpha.800'>{filteredService.name}</Text>
